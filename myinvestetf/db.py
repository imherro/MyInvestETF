    ...
    with closing(connect(db_path)) as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS etf_research_runs (
                ...
                valuation_mid REAL,
                valuation_high REAL,
                valuation_unit TEXT NOT NULL DEFAULT 'CNY/fund_share',
            ...
            CREATE INDEX IF NOT EXISTS idx_runs_code_date
                ON etf_research_runs(code, research_date);
...
def insert_research_run(conn: sqlite3.Connection, report: ETFResearchReport) -> int:
    if not isinstance(report, ETFResearchReport):
        run_id = report.get("run_id", "unknown") if isinstance(report, dict) else "unknown"
        etf_code = report.get("etf_code") or report.get("code", "unknown") if isinstance(report, dict) else "unknown"
        raise TypeError(
            "insert_research_run requires a validated ETFResearchReport "
            f"(run_id={run_id}, etf_code={etf_code})"
        )
    now = utc_now()
    payload = report.model_dump(mode="json")
    product = report.product_profile
    holdings = report.holdings_profile
    valuation = report.valuation
    fields = {
        "code": report.etf_code,
        "name": report.etf_name,
        ...
        "valuation_low": valuation.reference_value_low,
        "valuation_mid": valuation.reference_value_mid,
        "valuation_high": valuation.reference_value_high,
        ...
        "heavy_position_view": report.base_position_view,
    ...
    cursor = conn.execute(
        f"INSERT INTO etf_research_runs ({columns}) VALUES ({placeholders})",
        tuple(fields.values()),
    )
