...
ContrarianSignal 由 `core/strategy` 根据 DrawdownState、Regime v2、flow 因子、DecisionSignal 和 governance 生成；完整条件包括 `drawdown_extreme`、`regime_stress`、`volatility_stress`、`liquidity_stress` 和 `governance_allowed`。LLM 不手写反转概率、恐慌释放分、趋势衰竭分、触发条件或 final_view。
`enabled=true` 的硬门槛由系统判定：`drawdown_extreme && regime_stress && volatility_stress && governance_allowed`；`liquidity_stress` 参与概率和解释，但不是当前版本的硬门槛。自动化深研只需提供可靠的 ETF 行情序列、指数行情序列、流动性输入和数据缺口说明。
